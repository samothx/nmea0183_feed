...
use crate::Nmea0168State::{Encapsulation, Invalid};
...
enum Nmea0168State {
    ...
    Invalid,        // invalid - waiting for anything
...
impl Nmea0168Stm {
    fn new() -> Self {
        Self { msg: Nmea0168Msg::default(), count: 0, state: Encapsulation, param: String::new() }
    }

    fn add_byte(&mut self, byte: &u8) -> Option<Nmea0168Msg> {
        let state = self.state;
        if self.state != Invalid {
            // check for max message size
            self.count += 1;
            if self.count > 82 {
                self.state = Nmea0168State::Invalid;
            }
        }
            ...
            let result = match self.state {
                Nmea0168State::Encapsulation => {
                    match *byte {
                        ...
                        _ => self.state = Nmea0168State::Invalid
                    }
                },
                Nmea0168State::Talker => {
                    match *byte {
                        b'A'..=b'Z' => {
                ...
                Nmea0168State::MsgType => {
                    match *byte {
                        ...
                        FIELD => if self.msg.msgtype.len() == 3 {
                            self.state = Nmea0168State::Params;
                        } else {
                            self.state = Nmea0168State::Invalid;
                        }
                        AST => if self.msg.msgtype.len() == 3 {
                            self.state = Nmea0168State::Chksum;
                        } else {
                            self.state = Nmea0168State::Invalid;
                        }
                        CR => if self.msg.msgtype.len() == 3 {
...
impl Decoder for LineCodec {
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        let position = src.as_ref().iter().position(|b| {
           if let Some(msg) =  self.stm.add_byte(b) {
               result = Some(msg);
               true
           } else {
               false
...
pub fn get_codec<T>(port: T) -> Framed<impl AsyncRead,LineCodec>
    where T: AsyncRead + AsyncWrite + Sized
{
    // let mut codec = ;
...
fn to_string(buf: & BytesMut) -> String {
    ...
    buf.iter().for_each(|byte| str_buf.push_str(format!("{:02x} {},", byte, if  ((*byte) as char).is_control() {
        '?'
    } else {
        (*byte) as char
    }).as_str()));
